...
void save_data(student* personal_data, int student_count, char* file_name) {
    ...
    for (int i = 0; i < student_count; i++) {
            fprintf(pt, "%s %s %s ", personal_data[i].name, personal_data[i].surname, personal_data[i].patronymic);
            ...
            for (int j = 0; j < personal_data[i].number_of_discipline; j++) {
                fprintf(pt, "%s - %d ", personal_data[i].marks[j].discipline, personal_data[i].marks[j].mark);
            }
            fprintf(pt, "%d ", personal_data[i].number_of_events);
            for (int j = 0; j < personal_data[i].number_of_events; j++) {
                fprintf(pt, "%s %u ", personal_data[i].state[j].date, personal_data[i].state[j].current_state);
            }
...
void read_student_info(FILE* pt, student *personal_data, int student_count) {
    ...
    if (fscanf(pt, "%s %s %s ", personal_data[student_count].name, personal_data[student_count].surname, personal_data[student_count].patronymic) != 3) {
...
void read_student_marks(FILE* pt, student *personal_data, int student_count) {
    ...
    if (fscanf(pt, "%d ", &personal_data[student_count].number_of_discipline) != 1) {
        printf("Error reading number of disciplines.\n");
        exit(EXIT_FAILURE);
    }

    personal_data[student_count].marks = malloc(personal_data[student_count].number_of_discipline * sizeof(marks_and_discipline));
    if (personal_data[student_count].marks == NULL) {
        printf("Memory allocation error.\n");
        exit(EXIT_FAILURE);
    }
    for (int i = 0; i < personal_data[student_count].number_of_discipline; i++) {
        personal_data[student_count].marks[i].discipline = malloc(25 * sizeof(char));
        if (personal_data[student_count].marks[i].discipline == NULL) {
            printf("Memory allocation error.\n");
            exit(EXIT_FAILURE);
        }
        if (fscanf(pt, "%s - %d ", personal_data[student_count].marks[i].discipline, &personal_data[student_count].marks[i].mark) != 2) {
...
void read_student_events(FILE* pt, student *personal_data, int student_count) {
    if (fscanf(pt, "%d ", &personal_data[student_count].number_of_events) != 1) {
        printf("Error reading number of events.\n");
        exit(EXIT_FAILURE);
    }
    personal_data[student_count].state = malloc(personal_data[student_count].number_of_events * sizeof(event));
    if (personal_data[student_count].state == NULL) {
        printf("Memory allocation error.\n");
        exit(EXIT_FAILURE);
    }
    for (int i = 0; i < personal_data[student_count].number_of_events; i++) {
        if (fscanf(pt, "%s %u ", personal_data[student_count].state[i].date, &personal_data[student_count].state[i].current_state) != 2) {
            printf("Error reading date and current_state.\n");
            exit(EXIT_FAILURE);
        }
    }
    for (int i = 0; i < 1; i++) {
        if (fscanf(pt, "%u ", &personal_data[student_count].state[i].listed_or_not) != 1) {
...
void printf_file(student* personal_data, int n) {
    ...
    for(int i = 0; i < n; i++) {
        printf("Profile: %s %s %s\n", personal_data[i].name, personal_data[i].surname, personal_data[i].patronymic);
        printf("Number: %d\n", personal_data[i].number);
        for(int j = 0; j < personal_data[i].number_of_discipline; j++) {
            printf("Discipline: %s - %d\n", personal_data[i].marks[j].discipline, personal_data[i].marks[j].mark);
        }
        for(int j = 0 ; j < personal_data[i].number_of_events; j++) {
            printf("Event %d date %s state: %s\n", j + 1,personal_data[i].state[j].date, eve[personal_data[i].state[j].current_state]);
        }
